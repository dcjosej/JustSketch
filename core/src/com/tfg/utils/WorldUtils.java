...
//			System.out.println("Center: " + dir.cpy().scl(0.5f));
			...
			Rectangle rectangle = new Rectangle(point.x, point.y, distance, Constants.THICKNESS);
			FixtureStrokeUserData fixtureStroke = new FixtureStrokeUserData(rectangle, angle);
...
//	}
	...
	public static Body createPlatformBody(World world, Vector2 position){
		...
		PolygonShape shape =  new PolygonShape();
		shape.setAsBox(4, 1, position.scl(0.5f), 0f);
	...
	public static Body createBall(World world, Vector2 position){
		...
		res.setLinearVelocity(Constants.BALL_LINEAR_VELOCITY, res.getLinearVelocity().y);
